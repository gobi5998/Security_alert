...
class FileUploadService {
  ...
  static Map<String, dynamic> _createFileData(Map<String, dynamic> response) {
    return {
      'fileId': response['_id']?['\$oid'] ?? response['_id'],
      'url': response['s3Url'],
      'key': response['s3Key'],
  ...
  static Future<Map<String, dynamic>?> uploadFile(
    ...
    try {
      ...
      var response = await _dio.post(
        'https://a675e27c6222.ngrok-free.app/file-upload/threads/$fileType?reportId=$reportId',
        data: formData,
        options: Options(
          headers: {
  ...
  Future<List<Map<String, dynamic>>> triggerUpload() async {
    ...
    try {
      ...
      if (selectedImages.isNotEmpty || selectedDocuments.isNotEmpty || selectedVoiceFiles.isNotEmpty) {
        ...
        var uploadedFiles = await FileUploadService.uploadFiles(
          allFiles,
          widget.reportId,
