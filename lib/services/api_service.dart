...
class ApiService {
  ...
  Future<String?> _getAccessToken() async {
    try {
      ...
      // Fallback to JWT service if primary storage fails
      if (token == null || token.isEmpty) {
        print(
          'API Service - Primary token storage empty, trying JWT service...',
        );
  ...
  Future<List<Map<String, dynamic>>> fetchReportCategories() async {
    try {
      print(
        '🔍 Fetching report categories from: ${_dioService.mainApi.options.baseUrl}/report-category',
      );
      final response = await _dioService.get('/report-category');
  ...
  Future<List<Map<String, dynamic>>> fetchReportTypes() async {
    try {
      print(
        '🔍 Fetching report types from: ${_dioService.mainApi.options.baseUrl}/report-type',
      );
      final response = await _dioService.get('/report-type');
  ...
  Future<Map<String, dynamic>?> fetchCategoryById(String categoryId) async {
    try {
      final response = await _dioService.get('/report-category/$categoryId');
      return response.data;
    } catch (e) {
  ...
  Future<Map<String, dynamic>?> fetchTypeById(String typeId) async {
    try {
      final response = await _dioService.get('/report-type/$typeId');
      return response.data;
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> fetchReportsWithFilter(
    ...
    try {
      ...
      final response = await _dioService.get(filter.buildUrl());
    ...
    try {
      ...
      if (categoryIds != null && categoryIds.isNotEmpty) {
        ...
        if (categoryIds.length == 1) {
          // Send as single value if only one category
          queryParams['reportCategoryId'] = categoryIds.first;
          queryParams['categoryIds'] = categoryIds.first;
        } else {
          // Send as array if multiple categories
          queryParams['reportCategoryId'] = categoryIds;
          queryParams['categoryIds'] = categoryIds;
        }
        print('🔍 API Debug - categoryIds: $categoryIds');
      }
      if (typeIds != null && typeIds.isNotEmpty) {
        // Try different parameter names that the backend might expect
        if (typeIds.length == 1) {
          // Send as single value if only one type
          queryParams['reportTypeId'] = typeIds.first;
          queryParams['typeIds'] = typeIds.first;
        } else {
          // Send as array if multiple types
          queryParams['reportTypeId'] = typeIds;
          queryParams['typeIds'] = typeIds;
        }
        print('🔍 API Debug - typeIds: $typeIds');
      }
      if (severityLevels != null && severityLevels.isNotEmpty) {
        // Send alert level names to backend (not IDs)
        queryParams['alertLevels'] = severityLevels;
        print('🔍 API Debug - severityLevels: $severityLevels');
        print('🔍 API Debug - queryParams after adding alertLevels: $queryParams');
      } else {
        print('🔍 API Debug - severityLevels is null or empty: $severityLevels');
      }

      print('🔍 API Debug - Final queryParams: $queryParams');
      print('🔍 API Debug - Making request to /reports with parameters: $queryParams');
  ...
  Future<List<Map<String, dynamic>>> fetchAllReports() async {
    try {
      ...
      final response = await _dioService.get('/reports');
